...
class StatsService {
    async getProjectStats(projectId) {
        const totalResult = await (0, database_1.query)('SELECT COUNT(*) as count FROM submissions WHERE project_id = $1', [projectId]);
        const statusResult = await (0, database_1.query)(`SELECT status, COUNT(*) as count 
       FROM submissions 
       WHERE project_id = $1 
       GROUP BY status`, [projectId]);
        ...
        const avgReviewTimeResult = await (0, database_1.query)(`SELECT AVG(EXTRACT(EPOCH FROM (r.created_at - s.created_at))/3600) as avg_hours
       FROM reviews r
       JOIN submissions s ON r.submission_id = s.id
       WHERE s.project_id = $1 AND r.status IN ('approved', 'changes_requested')`, [projectId]);
        ...
        const approvalRate = statusCounts.approved + statusCounts.changes_requested > 0
            ...
            : 0;
        const activeReviewersResult = await (0, database_1.query)(`SELECT r.reviewer_id as user_id, u.name, COUNT(*) as review_count
       FROM reviews r
       JOIN submissions s ON r.submission_id = s.id
       ...
       LIMIT 5`, [projectId]);
        const mostCommentedResult = await (0, database_1.query)(`SELECT s.id as submission_id, s.title, COUNT(c.id) as comment_count
       FROM submissions s
       ...
       LIMIT 1`, [projectId]);
        ...
        return {
            ...
            most_active_reviewers: activeReviewersResult.rows.map((row) => ({
                user_id: row.user_id,
                name: row.name,
